/* PURPOSE: Independent Study CS - solve isd(P_n) for n bounded above
 * 
 * Runtime - successful
 * 
 * Usage: input a single positive integer >=3, the length of the path.
 */
...
void search(dsu DSU, int n, int isd, int considered = -1) { //endval is -(isd+1)/2    
    ...
    if(considered < 0) {
        //lower bound on T \setminus M if a_1 - 1 \in T
        int lo_tm = DSU.t_setminus_m(max(DSU.minlabel(), considered+DSU.effective_a_r1()),DSU.maxlabel()); 
        if(upperbound >= lo_tm) {
            search(DSU, n, isd, nextval);
        }
    } else  {
        //lower bound on T \setminus M if a_{n+t+1} + 1 \in T
        int hi_tm = DSU.t_setminus_m(DSU.minlabel()+DSU.effective_a_r1(), min(DSU.maxlabel()+DSU.effective_a_r1(), considered)); //lower bound on T \setminus M
        if(upperbound >= hi_tm) {
            search(DSU, n, isd, nextval);
        }
    }
    ...
    if(considered < 0) {
        //lower bound on T \setminus M if a_1 - 1 \in L
        ...
        //lower bound on T \setminus M if a_{n+t+1} + 1 \in L
...
int main() {
    ...
    //test isd in range [2n-5, 2n-2], the bounds from Li '22
    int isd = 2*n-5;
    for(; isd <= 2*n-2; isd++) {
        dsu DSU {isd};
        search(DSU, n, isd, -1);
    ...
    cout << "Runtime: " << (double) chrono::duration_cast<chrono::nanoseconds>(end_time    - start_time).count()/1000000000. << " seconds\n";
}